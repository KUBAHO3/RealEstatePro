...
const PropertyDetails = ({ PropertyDetails:{ price, rentFrequency, rooms, title, baths, area, agency, isVerified, description, type, purpose, furnishingStatus, amenities, photos, }}) =>(
    <Box maxWidth="1000px" margin="auto" p="4">
        ...
        <Flex alignItems="center" paddingTop="2" justifyContent="space-between">
            ...
            <Box>
                <Avatar size="sm" paddingRight="1" src={agency?.logo?.url}></Avatar>
            </Box>
        </Flex>
               ...
               <Flex flexWrap="wrap">
                    {amenities.map((item)=>(
                        item.amenities.map((amenity)=>(
                            <Text fontWeight="bold" color="blue.400" fontSize="15" p="2" bg="gray.200" m="1"
                                    borderRadius="5" key={amenity.text}>{amenity.text}</Text>
                        ))
                    ))}
               </Flex>
        </Box>
